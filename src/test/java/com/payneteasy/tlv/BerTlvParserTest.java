...
public class BerTlvParserTest {
    ...
    public void test_parsing_11_sample() {
        String hex = "3F 0A 0B 01 09 45 67 89 10 11 12 13 14 15 16"; // Tag: 3F 0A, Length: 0B, Value: 01 23 45 67 89 10
                                                                     // 11 12 13 14 15
        byte[] bytes = HexUtil.parseHex(hex);
        BerTlvParser parser = new BerTlvParser(LOG); // 로거 추가
        BerTlvs tlvs = parser.parse(bytes, 0, bytes.length);
        ...
        BerTlv tlv2 = tlv.getValues().get(0);
        Assert.assertEquals(new BerTag(0x01), tlv2.getTag());
        Assert.assertTrue(!tlv2.isConstructed());
